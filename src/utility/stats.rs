...
impl PerfStats {
    ...
    pub fn record_pass1_done(&self, phrase_id: u32, rtf: f32, audio_dur_s: f32, text_len: usize) {
        if let Ok(mut acc) = self.pass1_rtf_acc.lock() {
            *acc += rtf as f64;
        }
        self.pass1_count.fetch_add(1, Ordering::Relaxed);
        if let Ok(mut acc) = self.audio_dur_acc.lock() {
            *acc += audio_dur_s as f64;
        }
    ...
    pub fn record_pass2_done(&self, phrase_id: u32, rtf: f32, text_len: usize) {
        if let Ok(mut acc) = self.pass2_rtf_acc.lock() {
            *acc += rtf as f64;
        }
    ...
    pub fn reset(&self) {
        ...
        if let Ok(mut a) = self.pass1_rtf_acc.lock() {
            *a = 0.0;
        }
        if let Ok(mut a) = self.pass2_rtf_acc.lock() {
            *a = 0.0;
        }
        if let Ok(mut a) = self.audio_dur_acc.lock() {
            *a = 0.0;
        }
        if let Ok(mut q) = self.pipeline_events.lock() {
            q.clear();
